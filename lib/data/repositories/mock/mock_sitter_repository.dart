...
class MockSitterRepository implements ISitterRepository {
  static final List<SitterCard> _sitters = _buildMockSitters();
  ...
  Future<List<SitterCard>> searchSitters({
    ...
    if (filter.maxHourlyRate != null) {
      results = results.where((s) => s.profile.hourlyRate <= filter.maxHourlyRate!).toList();
    }
    ...
    results.sort((a, b) => switch (filter.sortBy) {
          ...
          SitterSortBy.lowestPrice => a.profile.hourlyRate.compareTo(b.profile.hourlyRate),
